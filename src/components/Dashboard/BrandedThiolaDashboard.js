...
class BrandedThiolaDashboard extends Component {
    ...
    componentWillMount=() =>{
        document.title = 'Branded Thiola Dashboard'
    }
    componentDidMount=()=>{
		...
		var tableaueDisplayOptions =
		{
          ...
          var data= axios({
                ...
                //tableaueVizURL = part1+response.data+process.env.REACT_APP_TABLEAU_BRAND_THIOLA_URL
                tableaueVizURL = process.env.REACT_APP_TABLEAU_BASE_URL+process.env.REACT_APP_TABLEAU_BRAND_THIOLA_URL
            ...
            <>
                ...
                <div className="container-fluid main mt-0" >
                    <div className="row title-row">
                        <h2 className="page-title">Branded Thiola Dashboard</h2>
                                ...
                                <nav>
                                    <div className="nav nav-tabs" id="nav-tab" role="tablist">
                                        <a className="nav-item nav-link active"  data-toggle="tab" href="#exec"
                                            role="tab" aria-controls="nav-home" aria-selected="true" onClick={()=>this.getCurrentSheet('General Performance')}>General Performance</a>
                                        <a className="nav-item nav-link"  data-toggle="tab" href="#exec"
                                            role="tab" aria-controls="nav-home" aria-selected="true" onClick={()=>this.getCurrentSheet('Patient')}>Patient</a>
                                        <a className="nav-item nav-link"  data-toggle="tab" href="#exec"
                                            role="tab" aria-controls="nav-profile" aria-selected="false" onClick={()=>this.getCurrentSheet('Sales Unit')}>Sales Unit</a>
                                        <a className="nav-item nav-link"  data-toggle="tab" href="#exec"
                                            role="tab" aria-controls="nav-contact" aria-selected="false" onClick={()=>this.getCurrentSheet('Payer')}>Payer</a>
                                    </div>
                                </nav>
...
export default connect(mapStateToProps)(withRouter(BrandedThiolaDashboard));
